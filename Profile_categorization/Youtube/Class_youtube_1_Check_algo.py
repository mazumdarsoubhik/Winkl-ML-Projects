...
w = models.KeyedVectors.load_word2vec_format("F:\Pretrained Models\GoogleNews-vectors-negative300.bin.gz", binary=True)
...
avoidwords = ['youtube', 'welcome', 'official', 'love', 'pan', 'go', 'click', 'facebook', 'subscribe', 'subscribing', 'verified',
                ...
                'karna', 'ho', 'ap', 'aap', 'liked', 'join', 'ke', 'apna', 'liye', 'mai', 'hoga', 'ya', 'logo', 'ki', 'gaya']
...
categories_list = [
                    ...
                    ['psychology', 'motivation', 'inspire', 'happiness', 'mind'],
...
x = requests.get('http://44.229.68.155/get_youtube_uncategorized_channels?current_id=15245&limit=5', headers={'Authorization': 'Token ruor7REQi9KJz6wIQKDXvwtt'})
...
while(len(data['channel']) !=0):
    try:
        ...
        last_id = int(dfnew['youtube_channel_id'].iloc[-1])
        ...
        for i in range(len(dfnew)):
            ...
            try:
                ...
                print(len(video_info_array),total_posts)
                ...
                print("ID no. {} Done! Total {} ids done".format(youtube_channel_id,idsdone))
                idsdone = idsdone +1
        ...
        # Request new page
        x = requests.get('http://44.229.68.155/get_youtube_uncategorized_channels?current_id=' + str(last_id + random.randint(2,8)) + '&limit=5', headers= {'Authorization': 'Token ruor7REQi9KJz6wIQKDXvwtt'})
        data = x.json()
        df = pd.DataFrame(data['channel'])
        status = x.status_code
