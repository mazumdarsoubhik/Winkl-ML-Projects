...
def compute2(caption,category_list,category,top =3):
    ...
    if len_keyword/2 <100 and len_keyword > 0:
        response_keywords = keywords[:len_keyword/2]
    elif len_keyword > 198:
    ...
    try:
        ...
        for i in range(len(dfnew)):
            ...
            try:
                ...
                logfile("ID no. {} Done! Total {} ids done".format(userid,idsdone))
                
            except Exception as Argument:
                # exc_type, exc_obj, exc_tb = sys.exc_info()
                # creating/opening a file 
                errorfile("Userid\t"+str(userid)+"\t: "+str(Argument))
                # f.write("Line number {exc_tb.tb_lineno} "+"Userid\t"+str(userid)+"\t: "+str(Argument)+str("\n")) 
                print("Userid\t"+str(userid)+"\t: "+str(Argument))
        ...
        pages = pages +1
        toc = time.time()
        logfile(txt.format(pages,last_id,last_offset,toc-new_tic))
        print(txt.format(pages,last_id,last_offset,toc-new_tic))
        # Request new page
        x = requests.get('http://44.229.68.155/insta_users/get_uncategorized_accounts_v2?limit=10&offset={}'.format(last_offset), headers={'Authorization': 'Token ruor7REQi9KJz6wIQKDXvwtt'})
        data = x.json()
        df = pd.DataFrame(data['users'])
        status = x.status_code
        print("Requested new page! last_offset{}".format(last_offset))
        logfile("Requested new page! last_offset{}".format(last_offset))

    except Exception as Argument:
        errorfile("Page no "+str(pages)+" with last_offset {last_offset}"+"\t"+str(Argument))
